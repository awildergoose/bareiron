...
void handlePlayerDisconnect(int client_fd) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    for (int j = 0; j < MAX_PLAYERS; j++) {
      if (player_data[j].client_fd == client_fd)
        continue;
      if (player_data[j].flags & 0x20)
        continue;
      // Send chat message
      sc_systemChat(player_data[j].client_fd, (char *)recv_buffer,
                    14 + player_name_len);
...
void handlePlayerJoin(PlayerData *player) {
  ...
  // Inform other clients (and the joining client) of the player's name and
  // entity
  for (int i = 0; i < MAX_PLAYERS; i++) {
...
uint8_t clientSlotToServerSlot(int window_id, uint8_t slot) {
  ...
  if (window_id == 0) { // player inventory
    ...
    if (slot >= 10 && slot <= 45)
      return clientSlotToServerSlot(0, slot - 1);
    ...
    if (slot >= 3 && slot <= 38)
      return clientSlotToServerSlot(0, slot + 6);
    ...
    if (slot >= 27 && slot <= 62)
      return clientSlotToServerSlot(0, slot - 18);
  }
#endif
...
int givePlayerItem(PlayerData *player, uint16_t item, uint8_t count) {
  ...
  for (int i = 0; i < 41; i++) {
    if (player->inventory_items[i] == item &&
        player->inventory_count[i] <= stack_size - count) {
      slot = i;
      break;
    }
  }
  ...
  sc_setContainerSlot(player->client_fd, 0, serverSlotToClientSlot(0, slot),
                      player->inventory_count[slot], item);
...
void spawnPlayer(PlayerData *player) {
  ...
  sc_synchronizePlayerPosition(player->client_fd, spawn_x, spawn_y, spawn_z,
  ...
  for (uint8_t i = 0; i < 41; i++) {
    sc_setContainerSlot(player->client_fd, 0, serverSlotToClientSlot(0, i),
...
void broadcastPlayerMetadata(PlayerData *player) {
  ...
  if (sneaking)
    entity_bit_mask |= 0x02;
  if (sprinting)
    entity_bit_mask |= 0x08;
...
uint8_t getBlockChange(short x, uint8_t y, short z) {
  for (int i = 0; i < block_changes_count; i++) {
    if (block_changes[i].block == 0xFF)
      continue;
    if (block_changes[i].x == x && block_changes[i].y == y &&
        block_changes[i].z == z)
...
void failBlockChange(short x, uint8_t y, short z, uint8_t block) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    sc_systemChat(
        ...
        "Block changes limit exceeded. Restore original terrain to continue.",
        67);
  }
}
...
uint8_t makeBlockChange(short x, uint8_t y, short z, uint8_t block) {
  ...
  ChunkAnchor anchor = {x / CHUNK_SIZE, z / CHUNK_SIZE};
  if (x % CHUNK_SIZE < 0)
    anchor.x--;
  if (z % CHUNK_SIZE < 0)
    anchor.z--;
...
// Returns the result of mining a block, taking into account the block type and
// tools Probability numbers obtained with this formula: N = floor(P * 32 ^ 2)
uint16_t getMiningResult(uint16_t held_item, uint8_t block) {

  switch (block) {

  case B_oak_leaves:
    if (held_item == I_shears)
      return I_oak_leaves;
  ...
  case B_coal_block:
    ...
    if (held_item != I_wooden_pickaxe && held_item != I_stone_pickaxe &&
  ...
  case B_diamond_ore:
    ...
    if (held_item != I_iron_pickaxe && held_item != I_golden_pickaxe &&
        held_item != I_diamond_pickaxe && held_item != I_netherite_pickaxe)
      return 0;
    break;
  ...
  case B_snow:
    ...
    if (held_item != I_wooden_shovel && held_item != I_stone_shovel &&
...
void bumpToolDurability(PlayerData *player) {
  ...
  uint32_t r = fast_rand();
  if (((held_item == I_wooden_pickaxe || held_item == I_wooden_axe ||
        held_item == I_wooden_shovel) &&
       r < 72796055) ||
      ((held_item == I_stone_pickaxe || held_item == I_stone_axe ||
        held_item == I_stone_shovel) &&
       r < 32786009) ||
      ((held_item == I_iron_pickaxe || held_item == I_iron_axe ||
        held_item == I_iron_shovel) &&
       r < 17179869) ||
      ((held_item == I_golden_pickaxe || held_item == I_golden_axe ||
        held_item == I_golden_shovel) &&
       r < 134217728) ||
      ((held_item == I_diamond_pickaxe || held_item == I_diamond_axe ||
        held_item == I_diamond_shovel) &&
       r < 2751420) ||
      ((held_item == I_netherite_pickaxe || held_item == I_netherite_axe ||
    ...
    sc_setContainerSlot(player->client_fd, 0,
                        serverSlotToClientSlot(0, player->hotbar), 0, 0);
  }
}
...
uint8_t isInstantlyMined(PlayerData *player, uint8_t block) {
  ...
  if (block == B_snow || block == B_snow_block)
    return (held_item == I_stone_shovel || held_item == I_iron_shovel ||
            held_item == I_diamond_shovel || held_item == I_netherite_shovel ||
            held_item == I_golden_shovel);
  ...
  return (block == B_dead_bush || block == B_short_grass || block == B_torch ||
...
uint8_t isColumnBlock(uint8_t block) {
  return (block == B_snow || block == B_moss_carpet || block == B_cactus ||
...
uint8_t isPassableBlock(uint8_t block) {
  return (block == B_air || (block >= B_water && block < B_water + 8) ||
          (block >= B_lava && block < B_lava + 4) || block == B_snow ||
          block == B_moss_carpet || block == B_short_grass ||
...
uint8_t isReplaceableBlock(uint8_t block) {
  return (block == B_air || (block >= B_water && block < B_water + 8) ||
          (block >= B_lava && block < B_lava + 4) || block == B_short_grass ||
          block == B_snow);
}
...
uint32_t isCompostItem(uint16_t item) {
  ...
  if ( // Compost chance: 30%
      item == I_oak_leaves || item == I_short_grass || item == I_wheat_seeds ||
      item == I_oak_sapling || item == I_moss_carpet)
    return 184070026;

  if ( // Compost chance: 50%
      item == I_cactus || item == I_sugar_cane)
    return 306783378;

  if ( // Compost chance: 65%
      item == I_apple || item == I_lily_pad)
    return 398818392;
...
uint8_t getItemStackSize(uint16_t item) {
  ...
  if (
      // Pickaxes
      item == I_wooden_pickaxe || item == I_stone_pickaxe ||
      item == I_iron_pickaxe || item == I_golden_pickaxe ||
      item == I_diamond_pickaxe || item == I_netherite_pickaxe ||
      // Axes
      item == I_wooden_axe || item == I_stone_axe || item == I_iron_axe ||
      item == I_golden_axe || item == I_diamond_axe ||
      item == I_netherite_axe ||
      // Shovels
      item == I_wooden_shovel || item == I_stone_shovel ||
      item == I_iron_shovel || item == I_golden_shovel ||
      item == I_diamond_shovel || item == I_netherite_shovel ||
      // Swords
      item == I_wooden_sword || item == I_stone_sword || item == I_iron_sword ||
      item == I_golden_sword || item == I_diamond_sword ||
      item == I_netherite_sword ||
      // Hoes
      item == I_wooden_hoe || item == I_stone_hoe || item == I_iron_hoe ||
      item == I_golden_hoe || item == I_diamond_hoe ||
...
uint8_t handlePlayerEating(PlayerData *player, uint8_t just_check) {
  ...
  *held_count -= 1;
  ...
  sc_setHealth(player->client_fd, player->health, player->hunger,
               player->saturation);
  sc_setContainerSlot(player->client_fd, 0,
                      serverSlotToClientSlot(0, player->hotbar), *held_count,
                      *held_item);
...
void handleFluidMovement(short x, uint8_t y, short z, uint8_t fluid,
  ...
  uint8_t adjacent[4] = {getBlockAt(x + 1, y, z), getBlockAt(x - 1, y, z),
...
void playPickupAnimation(PlayerData *player, uint16_t item, double x, double y,
                         double z) {
  ...
  sc_spawnEntity(player->client_fd, -1, (uint8_t *)player->name, 69, x + 0.5,
                 y + 0.5, z + 0.5, 0, 0);
...
void handlePlayerAction(PlayerData *player, int action, short x, short y,
                        short z) {
  ...
  if (action == 0 && !isInstantlyMined(player, block))
    return;

  // Don't continue if the block change failed
  if (makeBlockChange(x, y, z, 0))
    return;
...
void handlePlayerUseItem(PlayerData *player, short x, short y, short z,
                         uint8_t face) {
    ...
    if (target == B_crafting_table) {
      ...
      if (compost_chance != 0) {
        ...
        if ((*count -= 1) == 0)
          *item = 0;
        sc_setContainerSlot(player->client_fd, 0,
                            serverSlotToClientSlot(0, player->hotbar), *count,
                            *item);
  ...
  if (*item == I_bone_meal) {
    ...
    sc_setContainerSlot(player->client_fd, -2, serverSlotToClientSlot(0, slot),
                        1, *item);
    sc_setContainerSlot(player->client_fd, -2,
                        serverSlotToClientSlot(0, player->hotbar), 1,
                        prev_item);
    return;
  }
  ...
  if (!( // Is player in the way?
          !isPassableBlock(block) && x == player->x &&
          (y == player->y || y == player->y + 1) && z == player->z) &&
...
// Calculate fluid flow
  ...
  sc_setContainerSlot(player->client_fd, 0,
                      serverSlotToClientSlot(0, player->hotbar), *count, *item);
}
...
void interactEntity(int entity_id, int interactor_id) {
  ...
  PlayerData *player;
  if (getPlayerData(interactor_id, &player))
    return;
...
void hurtEntity(int entity_id, int attacker_id, uint8_t damage_type,
                uint8_t damage) {

  if (attacker_id > 0) { // Attacker is a player

    PlayerData *player;
    if (getPlayerData(attacker_id, &player))
      return;
    ...
    uint16_t held_item = player->inventory_items[player->hotbar];
    if (held_item == I_wooden_sword)
      damage *= 4;
    else if (held_item == I_golden_sword)
      damage *= 4;
    else if (held_item == I_stone_sword)
      damage *= 5;
    else if (held_item == I_iron_sword)
      damage *= 6;
    else if (held_item == I_diamond_sword)
      damage *= 7;
    else if (held_item == I_netherite_sword)
      damage *= 8;
  ...
  if (entity_id > 0) { // The attacked entity is a player
    ...
    PlayerData *player;
    if (getPlayerData(entity_id, &player))
      return;
    ...
    if (player->health <= effective_damage) {
      ...
      if (damage_type == D_fall && damage > 8) {
        ...
        strcpy((char *)recv_buffer + player_name_len,
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (entity_id >= 0) {
      ...
      sc_systemChat(client_fd, (char *)recv_buffer,
                    strlen((char *)recv_buffer));
    }
  }
}
...
// Effectively ties player movement updates to the tickrate
    ...
    // Tick damage from a cactus block if one is under/inside or around the
    // player.
    if (block == B_cactus ||
  ...
  for (int i = 0; i < MAX_MOBS; i++) {
    ...
    uint8_t passive = (mob_data[i].type == 25 || // Chicken
    ...
    if (!passive && (world_time < 13000 || world_time > 23460) &&
        mob_data[i].y > 48) {
    ...
    if (passive) {
      if (panic) {
        ...
        if (TICKS_PER_SECOND >= 4) {
          ...
          if (server_ticks % ticks_per_panic != 0)
            continue;
        }
        ...
        if (r % (4 * (unsigned int)TICKS_PER_SECOND) != 0)
          continue;
      }
    } else {
    ...
    for (int j = 0; j < MAX_PLAYERS; j++) {
      if (player_data[j].client_fd == -1)
        continue;
      uint16_t curr_dist = (abs(mob_data[i].x - player_data[j].x) +
    ...
    if (passive) { // Passive mob movement handling
      ...
      if (closest_player->x < old_x) {
        new_x -= 1;
        yaw = 64;
        if (closest_player->z < old_z) {
          new_z -= 1;
          yaw += 32;
        } else if (closest_player->z > old_z) {
          new_z += 1;
          yaw -= 32;
        }
    ...
    if ( // Validate diagonal movement
        new_x != old_x && new_z != old_z && !isPassableBlock(block_above) ||
    ...
    for (int j = 0; j < MAX_MOBS; j++) {
      if (j == i)
        continue;
      if (mob_data[j].type == 0)
        continue;
      if (mob_data[j].x == new_x && mob_data[j].z == new_z &&
...
void broadcastChestUpdate(int origin_fd, uint8_t *storage_ptr, uint16_t item,
                          uint8_t count, uint8_t slot) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (memcmp(player_data[i].craft_items, &storage_ptr, sizeof(storage_ptr)) !=
        0)
      continue;
