...
uint8_t getCornerHeight(uint32_t hash, uint8_t biome) {
  ...
  case W_mangrove_swamp: {
    height += ((hash % 3) + ((hash >> 4) % 3) + ((hash >> 8) % 3) +
               ((hash >> 12) % 3));
  ...
  case W_plains: {
    height +=
        ((hash & 3) + (hash >> 4 & 3) + (hash >> 8 & 3) + (hash >> 12 & 3));
    break;
  }
...
uint8_t getHeightAtFromAnchors(int rx, int rz, ChunkAnchor *anchor_ptr) {
  ...
  return interpolate(getCornerHeight(anchor_ptr[0].hash, anchor_ptr[0].biome),
                     getCornerHeight(anchor_ptr[1].hash, anchor_ptr[1].biome),
                     getCornerHeight(anchor_ptr[16 / CHUNK_SIZE + 1].hash,
                                     anchor_ptr[16 / CHUNK_SIZE + 1].biome),
                     getCornerHeight(anchor_ptr[16 / CHUNK_SIZE + 2].hash,
                                     anchor_ptr[16 / CHUNK_SIZE + 2].biome),
                     rx, rz);
}

uint8_t getHeightAtFromHash(int rx, int rz, int _x, int _z, uint32_t chunk_hash,
                            uint8_t biome) {
...
uint8_t getTerrainAtFromCache(int x, int y, int z, int rx, int rz,
                              ChunkAnchor anchor, ChunkFeature feature,
                              uint8_t height) {

  if (y >= 64 && y >= height && feature.y != 255)
    switch (anchor.biome) {
    case W_plains: { // Generate trees in the plains biome
      ...
      if (x == feature.x && z == feature.z) {
        if (y == feature.y - 1)
          return B_dirt;
        if (y >= feature.y && y < feature.y - feature.variant + 6)
          return B_oak_log;
      }
      ...
      if (dx < 3 && dz < 3 && y > feature.y - feature.variant + 2 &&
          y < feature.y - feature.variant + 5) {
        if (y == feature.y - feature.variant + 4 && dx == 2 && dz == 2)
          break;
        return B_oak_leaves;
      }
      if (dx < 2 && dz < 2 && y >= feature.y - feature.variant + 5 &&
          y <= feature.y - feature.variant + 6) {
        if (y == feature.y - feature.variant + 6 && dx == 1 && dz == 1)
          break;
        return B_oak_leaves;
      }
  ...
  if (height >= 63) {
    if (y == height) {
      if (anchor.biome == W_mangrove_swamp)
        return B_mud;
      if (anchor.biome == W_snowy_plains)
        return B_snowy_grass_block;
      if (anchor.biome == W_desert)
        return B_sand;
      if (anchor.biome == W_beach)
        return B_sand;
      return B_grass_block;
    }
  ...
  if (y <= height - 4) {
    ...
    if (y == ore_y) {
      // Since the ore Y coordinate is effectely a random number in range
      // [0;64), we use it in a bit shift with the chunk's anchor hash to get
      // another pseudo-random number for the ore's rarity.
  ...
  if (y <= height) {
    ...
    if (anchor.biome == W_mangrove_swamp)
      return B_mud;
    if (anchor.biome == W_beach && height > 64)
      return B_sandstone;
    return B_dirt;
  }
  // If all else failed, but we're below sea level, generate water (or ice)
  if (y == 63 && anchor.biome == W_snowy_plains)
    return B_ice;
  if (y < 64)
    return B_water;
...
ChunkFeature getFeatureFromAnchor(ChunkAnchor anchor) {
  ...
  if (anchor.biome != W_mangrove_swamp) {
    if (feature.x < 3 || feature.x > CHUNK_SIZE - 3)
      skip_feature = true;
    else if (feature.z < 3 || feature.z > CHUNK_SIZE - 3)
      skip_feature = true;
  }
  ...
  if (skip_feature) {
    ...
    feature.y = getHeightAtFromHash(mod_abs(feature.x, CHUNK_SIZE),
                                    mod_abs(feature.z, CHUNK_SIZE), anchor.x,
                                    anchor.z, anchor.hash, anchor.biome) +
                1;
...
uint8_t getTerrainAt(int x, int y, int z, ChunkAnchor anchor) {
  ...
  uint8_t height = getHeightAtFromHash(rx, rz, anchor.x, anchor.z, anchor.hash,
                                       anchor.biome);
...
uint8_t buildChunkSection(int cx, int cy, int cz) {
  ...
  for (int i = 0; i < 16; i++) {
    for (int j = 0; j < 16; j++) {
      anchor_index =
          (j / CHUNK_SIZE) + (i / CHUNK_SIZE) * (16 / CHUNK_SIZE + 1);
      ChunkAnchor *anchor_ptr = chunk_anchors + anchor_index;
      chunk_section_height[j][i] =
          getHeightAtFromAnchors(j % CHUNK_SIZE, i % CHUNK_SIZE, anchor_ptr);
    }
  }
  ...
  for (int j = 0; j < 4096; j += 8) {
    ...
    feature_index =
        (j % 16) / CHUNK_SIZE + (j / 16 % 16) / CHUNK_SIZE * (16 / CHUNK_SIZE);
    anchor_index = (j % 16) / CHUNK_SIZE +
                   (j / 16 % 16) / CHUNK_SIZE * (16 / CHUNK_SIZE + 1);
